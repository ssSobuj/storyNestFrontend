...
export default function EditStoryPage() {
  ...
  const { data: categories, isLoading: categoriesLoading } = useSwrFetcher(`/api/v1/categories`);
  const { data: storyResp, isLoading: storyLoading,mutate } = useSwrFetcher(`/api/v1/stories/${storyId}`);
  ...
  const { register, handleSubmit, setValue, watch, reset } = useForm<FormShape>({
    defaultValues: {
      title: "",
      content: "",
      category: "",
  ...
  const coverImagePreview = watch("coverImagePreview");

  // Initialize form when story data is available
  useEffect(() => {
    if (story && !isFormInitialized) {
      reset({
        title: story.title || "",
        content: story.content || "",
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setCoverImageFile(file);
      setValue("coverImagePreview", URL.createObjectURL(file));
    }
  };
  ...
  const onSubmit = async (data: FormShape) => {
    ...
    submissionData.append("content", data.content);
    submissionData.append("category", data.category);
    submissionData.append("tags", JSON.stringify(data.tags));
    if (coverImageFile) submissionData.append("coverImage", coverImageFile);
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                <Label htmlFor="title">Title *</Label>
                <Input  value={watch("title")}
    onChange={(e) => setValue("title", e.target.value)} required />
              </div>
              <div>
                <Label htmlFor="content">Content *</Label>
                <textarea
              ...
              <div>
                ...
                {categoriesLoading ? (
                  ...
                  <select
                    ...
                    defaultValue={story?.category?._id || ""}
                  >
